...
NodeEntry DefaultAggregateGradient(std::vector<NodeEntry>&& v) {
  if (v.size() == 1) {
    ...
    NodePtr agg_node = Node::Create();

    agg_node->attrs.op = Op::Get("__zero__");
    return NodeEntry{agg_node, 0, 0};
  } else {
    NodePtr agg_node = Node::Create();

    agg_node->attrs.op = Op::Get("__ewise_sum__");
    agg_node->inputs   = std::move(v);
    return NodeEntry{agg_node, 0, 0};
  }
}
...
NNVM_REGISTER_PASS(GradientV2)
    .describe("Return a gradient graph of src.attrs[\"ys\"] "
                "w.r.t. src.attrs[\"xs\"]")
    .set_body(GradientV2)
